...
class WhisperConfig(BaseModel):
    ...
    language: str = "auto"
    compute_type: Literal["int8", "float16", "float32"] = "float32"
    chunk_length: int = Field(
        default=60, ge=10, le=600, description="音频分割时长（秒）"
    )
    initial_prompt: str = Field(default="", description="初始提示词，提升转录质量")
    parallel_workers: int = Field(
        default=1, ge=1, le=4, description="并行转录工作数（长音频分段并行处理）"
    )
    ...
    align_model: str = Field(
        default="", description="强制使用指定对齐模型（空字符串表示自动选择）"
    )
    ...
    min_speakers: int | None = Field(
        default=None, ge=1, le=10, description="最小说话人数（可选）"
    )
    max_speakers: int | None = Field(
        default=None, ge=1, le=10, description="最大说话人数（可选）"
    )
...
class OutputConfig(BaseModel):
    ...
    md_filename_template: str = Field(
        default="{title}.md", description="Markdown文件名模板"
    )
...
class OpenAIConfig(BaseModel):
    ...
    max_tokens: int = Field(
        default=8192, ge=1000, le=32000, description="最大输出token数"
    )
    temperature: float = Field(default=0.1, ge=0.0, le=2.0, description="生成温度")
    request_timeout: int = Field(
        default=120, ge=10, le=600, description="请求超时时间(秒)"
    )
...
class StructuredMarkdownConfig(BaseModel):
    ...
    segment_length: int = Field(
        default=6000, ge=1000, description="长文本分段处理每段长度"
    )
    preserve_original: bool = Field(default=True, description="是否保留原始转录文本")
    auto_detect_domain: bool = Field(
        default=True, description="是否自动检测播客内容领域"
    )
...
class PodcliConfig(BaseModel):
    ...
    structured: StructuredMarkdownConfig = Field(
        default_factory=StructuredMarkdownConfig
    )
